...
1. Is it a fixed, checkable pattern: a banned API, an import shape, a file location? Put
   the enforcement in a linter, hook, or CI job, and keep it in the instruction file only
   when the agent must follow it before the check runs.
2. Does it apply everywhere in the repo, and is it short enough to read every session?
...
- Anything that does not change behavior versus the model's default, such as "write clean
  code" or "add tests."
...
- Anything the agent can find by looking, such as a function's parameter list, a config
  file's keys, or a command that's just the contents of a script. Cache only what looking
  cannot find.
